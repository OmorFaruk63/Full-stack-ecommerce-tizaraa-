...
export async function generateStaticParams() {
 return ["transcend-esd380c-500gb-type-c-portable-ssd"].map((slug) => ({
  slug,
 }));
}
...
export async function generateMetadata({
 ...
 const res: ProductDetailsType | null = await fetchDataSSR(
  `/api/product/${slug}`
 );
 if (!res) {
  return {
...
export default async function ProductDetailsPage({
 ...
 const { slug } = await params;
 const productData: ProductDetailsType | null = await fetchDataSSR(
  `/api/product/${slug}`
 );
